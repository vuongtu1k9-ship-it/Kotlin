...
export function registerInviteHandlers(io, socket) {
  const handleInviteSend = async ({ toUid, roomId } = {}, ack) => {
    try {
      ...
      logger.info(`[INVITE] Invitation sent from ${doc.fromUid} to ${target} for room ${rid}. Triggering push...`);
      sendNotification(target, pushPayload).catch(err => logger.error(`[INVITE] Push notification failed for ${target}:`, err.message));

      if (typeof ack === 'function') ack({ ok: true, invite: payload });
    } catch (e) {
      if (typeof ack === 'function') ack({ ok: false, error: 'INVITE_FAILED' });
      logger.error('[INVITE] invite:send exception:', e.message);
    }
  };
  ...
  const handleInviteRespond = async ({ id, action } = {}, ack) => {
    try {
      ...
      logger.info(`[INVITE] Invitation ${invId} ${status} by ${me}`);
      if (typeof ack === 'function') ack({ ok: true, invite: next });
    } catch (e) {
      if (typeof ack === 'function') ack({ ok: false, error: 'RESPOND_FAILED' });
      logger.error('[INVITE] invite:respond exception:', e.message);
    }
  };
