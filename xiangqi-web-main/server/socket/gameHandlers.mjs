...
export function registerGameHandlers(io, socket) {
  socket.on('game:move', async ({ roomId: rawRoomId, move, clientMoveIndex, lastServerMoveIndex } = {}, ack) => {
    ...
    if (!state) {
      logger.warn(`[GAME_HANDLERS] ROOM_NOT_FOUND (roomId: ${roomId})`);
      if (typeof ack === 'function') ack({ ok: false, error: 'ROOM_NOT_FOUND' });
      return;
    }
    if (state.finished) {
      logger.warn(`[GAME_HANDLERS] GAME_FINISHED (roomId: ${roomId})`);
    ...
    if (!side) {
      logger.warn(`[GAME_HANDLERS] NOT_IN_ROOM (roomId: ${roomId}, socket: ${socket.id})`);
    ...
    if (t) {
      ...
      logger.warn(`[GAME_HANDLERS] TIMEOUT (roomId: ${roomId}, loser: ${t.loser})`);
    ...
    if (typeof lastServerMoveIndex === 'number' && lastServerMoveIndex !== state.serverMoveIndex) {
      logger.warn(`[GAME_HANDLERS] OUT_OF_SYNC (roomId: ${roomId}, client: ${lastServerMoveIndex}, server: ${state.serverMoveIndex})`);
    ...
    if (side !== state.currentPlayer) {
      logger.warn(`[GAME_HANDLERS] NOT_YOUR_TURN (roomId: ${roomId}, side: ${side}, expected: ${state.currentPlayer})`);
    ...
    if (!move || !move.from || !move.to) {
      logger.warn(`[GAME_HANDLERS] BAD_MOVE payload (roomId: ${roomId}, move: ${JSON.stringify(move)})`);
    ...
    if (!isValidCoord(from) || !isValidCoord(to)) {
      logger.warn(`[GAME_HANDLERS] INVALID_COORDINATES (roomId: ${roomId}, from: ${JSON.stringify(from)}, to: ${JSON.stringify(to)})`);
    ...
    if (!piece) {
      logger.warn(`[GAME_HANDLERS] NO_PIECE at from: ${JSON.stringify(from)} (roomId: ${roomId})`);
      if (typeof ack === 'function') ack({ ok: false, error: 'NO_PIECE' });
      return;
    }
    if (piece.side !== side) {
      logger.warn(`[GAME_HANDLERS] NOT_YOUR_PIECE (roomId: ${roomId}, pieceSide: ${piece.side}, mySide: ${side})`);
      if (typeof ack === 'function') ack({ ok: false, error: 'NOT_YOUR_PIECE' });
      return;
    }

    if (!isLegalMove(piece, to, state.board)) {
      logger.warn(`[GAME_HANDLERS] ILLEGAL_MOVE logic rejected (roomId: ${roomId}, piece: ${piece.type}, from: ${JSON.stringify(from)}, to: ${JSON.stringify(to)})`);
    ...
    if (!applied.ok) {
      logger.warn(`[GAME_HANDLERS] ILLEGAL_MOVE apply fails (roomId: ${roomId}, error: ${applied.error})`);
    ...
    const givesCheck = isInCheck(opponentSide, state.board);
    ...
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (!p || p.side === opponentSide) continue; // Skip if not an opponent piece (Wait, p.side === opponentSide is what we WANT)
        // Correct logic: if p is opponent piece
        if (p.side !== opponentSide) continue;
    ...
    // Switch turn after persisting spent time
    ...
    if (!state.started && state.serverMoveIndex === 1) {
      ...
      state.winner = null;
      state.endedBy = null;
      logger.info(`[GAME_HANDLERS] Auto-starting match in room ${roomId} (first move made)`);
    ...
    try {
      ...
      logger.error('[GAME_HANDLERS] save move failed:', e);
    }
    ...
    const any = hasAnyLegalMove(state.currentPlayer, state.board);
    ...
    if (!any || (bothBots && moveLimitReached) || moveLimit60 || isRepetition || isInsufficient) {
      const inCheck = isInCheck(state.currentPlayer, state.board);
      state.finished = true;
      state.started = false;
      let drawReason = '';
      ...
      if ((bothBots && moveLimitReached && any) || (moveLimit60 && any)) {
          state.winner = null;
          state.endedBy = 'draw';
          drawReason = moveLimit60 ? 'Luật 60 nước đi (60-move rule)' : 'Giới hạn nước đi của Bot (Bot limit)';
          logger.info(`[GAME_HANDLERS] Match in room ${roomId} ended as DRAW (${drawReason})`);
          ...
          if (forbiddenSide) {
              ...
              drawReason = `Xử thua ${forbiddenSide === 'red' ? 'bên Đỏ' : 'bên Đen'} do vi phạm luật Chiếu dai/Đuổi dai (Perpetual Rule)`;
              logger.info(`[GAME_HANDLERS] Match in room ${roomId} ended as LOSS for ${forbiddenSide} (${drawReason})`);
          } else {
              state.winner = null;
              state.endedBy = 'draw';
              drawReason = 'Lặp lại 3 lần (Threefold Repetition)';
              logger.info(`[GAME_HANDLERS] Match in room ${roomId} ended as DRAW (${drawReason})`);
          }
      } else if (isInsufficient && any) {
          state.winner = null;
          state.endedBy = 'draw';
          drawReason = 'Hết quân tấn công (Insufficient Material)';
          logger.info(`[GAME_HANDLERS] Match in room ${roomId} ended as DRAW (${drawReason})`);
      } else {
      ...
      io.to(roomId).emit('game:over', {
        ...
        reason: drawReason, // Pass the detailed reason to the client
      });
