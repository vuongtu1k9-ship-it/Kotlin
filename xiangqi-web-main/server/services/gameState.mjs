...
export function createInitialState({ timeMode = 'standard', timeControl, board, setupId, tournamentId, playerUids, playerNames } = {}) {
  if (board) logger.info(`[GAME_STATE] createInitialState with CUSTOM board (setupId=${setupId})`);
  else logger.info(`[GAME_STATE] createInitialState with STANDARD board`);
  
  const now = Date.now();
  const tc =
    timeControl ||
    (timeMode === 'blitz'
      ? { totalMs: 3 * 60_000, perMoveMs: 30_000 }
      : timeMode === 'rapid'
        ? { totalMs: 10 * 60_000, perMoveMs: 60_000 }
        : timeMode === 'slow'
          ? { totalMs: 45 * 60_000, perMoveMs: 300_000 }
          : { totalMs: 20 * 60_000, perMoveMs: 120_000 }); // standard
...
export function startTurnClock(state, side) {
  ...
  const perMoveMs = state.timeControl?.perMoveMs ?? 120_000;
...
export function checkTimeout(state, now) {
  ...
  if (remaining <= 0) {
    logger.info(`[GAME_STATE] Timeout detected: ${side} total time expired (${remaining}ms)`);
    return { loser: side, reason: 'total' };
  }
  if (perMoveDeadlineAt && now > perMoveDeadlineAt) {
    logger.info(`[GAME_STATE] Timeout detected: ${side} per-move deadline expired (now=${now}, deadline=${perMoveDeadlineAt})`);
    return { loser: side, reason: 'perMove' };
  }
  return null;
}
