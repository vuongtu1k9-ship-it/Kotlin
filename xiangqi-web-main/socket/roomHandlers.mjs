...
export function registerRoomHandlers(io, socket) {
  ...
  socket.on('room:active_check', async () => {
    try {
      ...
      logger.error('[roomHandlers] room:active_check failed:', e);
    }
  });
  ...
  socket.on('room:create', async ({ timeMode, setupId, tournamentId, isPrivate, boardType, isFixed } = {}, ack) => {
    ...
    try {
      ...
      logger.info(`[room:create] roomId=${roomId} setupId=${setupId} user=${uid} (Guest: ${isGuest})`);
      ...
      if (setupId) {
        try {
          ...
          if (Array.isArray(b) && b.length === 10 && Array.isArray(b[0])) {
            ...
            logger.error('[roomHandlers] setup board invalid format:', typeof b);
          ...
          logger.error('room:create setup load failed', e);
          return;
        }
      }

      logger.debug(`[roomHandlers] Creating room: isPrivate=${isPrivate}, tournamentId=${tournamentId}, isFixed=${isFixed}, uid=${uid}`);
      ...
      logger.info(`[roomHandlers] Automated Invite PRE-CHECK: roomId=${roomId}, isPrivate=${st.isPrivate}, tournamentId=${st.tournamentId}, isFixed=${st.isFixed}`);
      if (!st.isPrivate && !st.tournamentId && !st.isFixed) {
        ...
        logger.info(`[roomHandlers] Triggering broadcastToIdleUsers for room ${roomId} by ${creatorName} (uid: ${uid})`);
        broadcastToIdleUsers(io, roomId, creatorName, st.timeMode, uid);
      }

      if (typeof ack === 'function') ack({ ok: true, roomId });
    } catch (e) {
      logger.error('[roomHandlers] room:create critical failure:', e);
  ...
  socket.on('lobby:list', async (ack) => {
    try {
      ...
      logger.error('lobby:list failed', e);
    }
  });
  ...
  socket.on('presence:list', (ack) => {
    try {
      ...
      logger.error('[roomHandlers] presence:list error', e);
  ...
  socket.on('room:watch', async ({ roomId } = {}, ack) => {
    try {
      ...
      getChatHistory(roomId).then(h => socket.emit('chat:history', h));
      ...
      logger.error('room:watch failed', e);
    }
  });
  ...
  socket.on('entity:join', async ({ type, id } = {}, ack) => {
    try {
      ...
      logger.error('entity:join failed', e);
